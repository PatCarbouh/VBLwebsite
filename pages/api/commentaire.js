...
export default async function handler(req, res) {
  ...
  try {
    const { name, email, subject, note, message, contactPermission } = req.body;

    // Log received data
    console.log('Form data received:', { name, email, subject, note, message, contactPermission });

    // Basic validation
    if (!name || !email || !subject || !message || note === undefined || isNaN(parseInt(note)) || parseInt(note) < 0 || parseInt(note) > 5 || contactPermission === undefined
       ) {
    ...
    try {
      const adminMailResponse = await transporter.sendMail({
        ...
        html: `
          ...
          <p><strong>Entreprise:</strong> ${subject}</p>
